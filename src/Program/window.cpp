...
void PWindow::UpdateTitle(std::string workspaceName, EWorkMode workMode, bool changed) {
    std::string title = "SplineAR - " + workspaceName;
    if (changed) {
        title += "*";
    }
    title += " - " + ENUM_WorkMode_GetName(workMode);
